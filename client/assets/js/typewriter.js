(function () {
  ...
  const TypewriterPrototype = window.Typewriter.prototype;
  ...
  TypewriterPrototype.typeString = function (string) {
    ...
    const string_chars = this._getCharacters(string);
  ...
  TypewriterPrototype.typeOutAllStrings = function () {
    const characters_array = this._getStringsAsCharsArray();
    if (characters_array.length === 1) {
      this._typeCharacters(characters_array[0]);
    } else {
      for (let i = 0, length = characters_array.length; i < length; i++) {
  ...
  TypewriterPrototype._deletingCharAnimationFrame = function () {
    const self = this;
    let delete_speed = this.options.deleteSpeed;
    const typewriter_wrapper_class_name = self.options.wrapperClassName;
    const current_typed_char_ids = self._settings.currentTypedCharacters;
    let char_amount_to_delete = self._settings.charAmountToDelete;
    ...
    setTimeout(() => {
      if (self._settings.charAmountToDelete) {
        const last_typed_char_index = current_typed_char_ids.length - 1;
        const get_last_typed_char = current_typed_char_ids[last_typed_char_index];
        self._settings.currentTypedCharacters.splice(last_typed_char_index, 1);
        const char_to_delete_el = document.getElementById(get_last_typed_char);
        if (char_to_delete_el) {
          const typewriter_wrapper_el = self.el.querySelector(`.${typewriter_wrapper_class_name}`);
  ...
  TypewriterPrototype._startEventLoop = function () {
    ...
    if (!this._settings.eventRunning) {
      if (this._settings.eventQue.length > 0) {
        ...
        const first_event = this._settings.eventQue[0];
  ...
  TypewriterPrototype._setupTypwriterWrapper = function () {
    const typewriter_wrapper_class_name = this.options.wrapperClassName;
    const typewriter_wrapper = document.createElement('span');
  ...
  TypewriterPrototype._typingAnimationFrame = function (total_items) {
    const self = this;
    let typing_speed = this.options.typingSpeed;
    const typewriter_wrapper_class_name = self.options.wrapperClassName;
    ...
    setTimeout(() => {
      const el_inner_html = self.el.innerHTML;
      const item_to_type = self._settings.stringToTypeHTMLArray[0];
  ...
  TypewriterPrototype._convertCharsToHTML = function (chars) {
    const chars_html_wrap_array = [];
    const char_class_name = this.options.charSpanClassName;
    const chars_array = chars[0];
    for (let i = 0, length = chars_array.length; i < length; i++) {
      const char_element = document.createElement('span');
      const char_id = this._generateUniqueID();
  ...
  TypewriterPrototype._getStringsAsCharsArray = function () {
    const strings_array_check = this.options.strings instanceof Array;
    const strings_string_check = typeof this.options.strings === 'string';
    ...
    const strings_chars_array = [];
    for (let i = 0, length = this.options.strings.length; i < length; i++) {
      const string_chars = this._getCharacters(this.options.strings[i]);
      if (!string_chars) {
  ...
  TypewriterPrototype._cursorAnimationFrame = function () {
    if (!this._settings.cursorAnimationPaused) {
      const blink_speed = this.options.blinkSpeed;
      const opacity_amount = (1 / 1000) * blink_speed;
      const cursor_el = this.el.querySelector('.typewriter-cursor');
  ...
  TypewriterPrototype._startCursorAnimation = function () {
    ...
    const cursor_class_name = this.options.cursorClassName;
    const cursor_element = document.createElement('span');
  ...
  TypewriterPrototype._randomID = function () {
    ...
    for (let i = 0; i < this._randomInteger(5, 15); i++) {
      text += possible.charAt(Math.floor(Math.random() * possible.length));
    }
    return text;
  };
  TypewriterPrototype._generateUniqueID = function () {
    const temp_id = this._randomID();
